...
import { sealEpoch, openShare, recoverEpoch, requestRecovery, recoveryOutcome, requestDetails, releaseShare } from '../src/index.js'
...
describe('fuzz: shares', () => {
  const sealed = sealEpoch({ epochKey, epochId: 'e1', keeperPrivateKey: keeper, members, threshold: 3 })
  it('a mutated wrap never opens and never hangs', () => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      let threw = false
      try { openShare(m, circle[sealed.indexOf(s)]!) } catch (e) { threw = e instanceof Error }
  ...
  it('random share sets never reconstruct the key by accident', () => {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      const out = recoverEpoch(bad)
      const same = Buffer.from(out).equals(Buffer.from(epochKey))
...
describe('fuzz: recovery', () => {
  it('garbage requests and ballots never release a share', async () => {
    const r = rng(33)
    const sealed = sealEpoch({ epochKey, epochId: 'e2', keeperPrivateKey: keeper, members, threshold: 3 })
    const share = openShare(sealed[0]!.wrap, circle[0]!)
    const later = Math.floor(Date.now() / 1000) + 99999
    for (let i = 0; i < 200; i++) {
      ...
      expect(() => releaseShare({ memberPrivateKey: circle[0]!, share, request: junk, ballots: [], members, now: later })).toThrow()
    }
    const request = await requestRecovery({ keeperPrivateKey: keeper, epochId: 'e2', members, delaySeconds: 3600, recoverTo: getPublicKey(generateSecretKey()) })
    for (let i = 0; i < 200; i++) {
